...
(* FIXME RT: move to... ?*)
lemma add_asserts:
  assumes not: "\<lbrace>Not o P\<rbrace> f \<lbrace>\<lambda>_. \<bottom>\<rbrace>"
  shows "\<lbrace>\<lambda>s. P s \<and> Q s\<rbrace> f \<lbrace>R\<rbrace> \<Longrightarrow> \<lbrace>Q\<rbrace> f \<lbrace>R\<rbrace>"
  apply (insert use_valid[OF _ not])
  apply (fastforce simp: valid_def)
  done

lemma blockedCancelIPC_asserts:
  "\<lbrace>Not o (obj_at' ((\<noteq>) IdleEP) (the (epBlocked st)) and K (\<exists>x. epBlocked st = Some x))\<rbrace>
   blockedCancelIPC st tptr rptrOpt
   \<lbrace>\<lambda>_. \<bottom>\<rbrace>"
  unfolding blockedCancelIPC_def getBlockingObject_def Let_def
  apply (wpsimp wp: haskell_assert_wp set_ep'.get_wp
            wp_del: hoare_vcg_prop)
...
lemma blockedCancelIPC_invs':
  ...
  apply (rule add_asserts[OF blockedCancelIPC_asserts])
